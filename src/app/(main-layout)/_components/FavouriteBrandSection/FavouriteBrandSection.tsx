          ...
          {favouriteBrands?.map((brand) => (
            <>
              <Link href={`brands/${brand?.brandName}`}>
                <div
                  key={brand?._id}
                  className="border hover:border-primary-light hover:bg-image-background transition-all duration-100 border-black-10 h-[60px] 2xl:h-[70px] relative rounded-lg"
                >
                  <Image
